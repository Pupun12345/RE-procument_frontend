...
export function Report() {
  ...
  const [tableFilter, setTableFilter] = useState<"monthly" | "weekly" | "yearly">("monthly");
  ...
  const getFilteredData = () => {
    if (tableFilter === "yearly") {
      // Show single row with totals
      return [{
        month: fiscalYear,
        monthNumber: 0,
        year: parseInt(fiscalYear.split("-")[0].replace("FY ", "")),
        headcount: monthlyData.length > 0 ? Math.round(monthlyData.reduce((sum, d) => sum + d.headcount, 0) / monthlyData.length) : 0,
        basicPay: totals.basicPay,
        hra: totals.hra,
        allowances: totals.allowances,
        grossSalary: totals.grossSalary,
        pfEmp: totals.pfEmp,
        esiEmp: totals.esiEmp,
        netSalary: totals.netSalary
      }];
    } else if (tableFilter === "weekly") {
      // Group monthly data into quarters (as weekly approximation)
      ...
      <div className="table-section">
        <div className="table-header">
          <div className="table-title-group">
            <h2 className="table-title">Detailed Breakdown</h2>
            <div className="filter-tabs">
              <button 
                className={`filter-tab ${tableFilter === 'monthly' ? 'active' : ''}`}
                onClick={() => setTableFilter('monthly')}
              >
                Monthly
              </button>
              <button 
                className={`filter-tab ${tableFilter === 'weekly' ? 'active' : ''}`}
                onClick={() => setTableFilter('weekly')}
              >
                Quarterly
              </button>
              <button 
                className={`filter-tab ${tableFilter === 'yearly' ? 'active' : ''}`}
                onClick={() => setTableFilter('yearly')}
              >
                Yearly
              </button>
            </div>
        ...
        <div className="table-footer">
          <span className="viewing-text">
            {tableFilter === 'yearly' && `Viewing yearly summary for ${fiscalYear}`}
            {tableFilter === 'weekly' && `Viewing ${getFilteredData().length} quarters`}
            {tableFilter === 'monthly' && `Viewing ${monthlyData.length} of 12 months`}
          </span>
        </div>
      </div>
    </div>
